...
namespace MoonSharp.Interpreter.Tests.EndToEnd
{
	...
	public class UserDataOverloadsTests
	{
		...
		private void RunTestOverload(string code, string expected, bool tupleExpected = false)
		{
			...
			if (tupleExpected)
			{
				Assert.Equal(DataType.Tuple, v.Type);
				v = v.Tuple[0];
			}
		...
		public void OverloadTest_WithoutObjects()
		{
			...
			Assert.Equal(DataType.Number, result.Tuple[0].Type);
			Assert.Equal(DataType.Number, result.Tuple[1].Type);
			Assert.Equal(1, result.Tuple[0].Number);
			Assert.Equal(22, result.Tuple[1].Number);
		}
#endif
