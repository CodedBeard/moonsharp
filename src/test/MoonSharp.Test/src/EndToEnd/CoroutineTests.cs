...
namespace MoonSharp.Interpreter.Tests.EndToEnd
{
	...
	public class CoroutineTests
	{
		...
		public void Coroutine_Basic()
		{
			...
			Assert.Equal("1-5;2-6;3-7;4-8;", res.String);
		}
		...
		public void Coroutine_ClrBoundaryHandling()
		{
			...
			Assert.Equal(DataType.Tuple, ret.Type);
			Assert.Equal(2, ret.Tuple.Length);
			Assert.Equal(DataType.Boolean, ret.Tuple[0].Type);
			Assert.Equal(false, ret.Tuple[0].Boolean);
			Assert.Equal(DataType.String, ret.Tuple[1].Type);
			Assert.True(ret.Tuple[1].String.EndsWith("attempt to yield across a CLR-call boundary"));
		}
