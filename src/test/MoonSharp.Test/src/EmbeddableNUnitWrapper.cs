...
namespace NUnit.Framework
{
	...
	public static class Assert
	{
		...
		{
			Assert.True((double)expected == other, message ?? string.Format("{0} was expected, {1} was returned", expected, other));
		}
		...
		public static void AreEqual(object expected, object other, string message = null)
		{
			if (expected is int && other is double) 
				...
				Assert.True(expected.Equals(other), message ?? string.Format("{0} was expected, {1} was returned", expected, other));
			else
				Assert.True(other == null, message ?? string.Format("null was expected, {0} was returned", other));
		}
