...
namespace MoonSharp.Interpreter.Tests.EndToEnd
{
	...
	public class ConfigPropertyAssignerTests
	{
		...
		private MyClass Test(string tableDef)
		{
			...
			NUnit.Framework.Assert.AreEqual(DataType.Table, table.Type);
		...
		public void ConfigProp_SimpleAssign()
		{
			...
			NUnit.Framework.Assert.AreEqual(x.MyNumber, 3);
			NUnit.Framework.Assert.AreEqual(x.MyString, "ciao");
			NUnit.Framework.Assert.AreEqual(x.NativeValue.Type, DataType.Function);
			NUnit.Framework.Assert.AreEqual(x.SubObj.MyNumber, 15);
			NUnit.Framework.Assert.AreEqual(x.SubObj.MyString, "hi");
			NUnit.Framework.Assert.IsNotNull(x.SomeTable);
