...
namespace MoonSharp.Interpreter.Tests.EndToEnd
{
	...
	public class UserDataOverloadsTests
	{
		...
		private void RunTestOverload(string code, string expected, bool tupleExpected = false)
		{
			...
			if (tupleExpected)
			{
				NUnit.Framework.Assert.AreEqual(DataType.Tuple, v.Type);
				v = v.Tuple[0];
			}

			NUnit.Framework.Assert.AreEqual(DataType.String, v.Type);
			NUnit.Framework.Assert.AreEqual(expected, v.String);
		}
		...
		{
            Assert.Throws<ScriptRuntimeException>(() => { // pollute cache
                RunTestOverload("o:method1(5)", "3");
		...
		public void OverloadTest_WithoutObjects()
		{
			...
			NUnit.Framework.Assert.AreEqual(DataType.Tuple, result.Type);
			NUnit.Framework.Assert.AreEqual(DataType.Number, result.Tuple[0].Type);
			NUnit.Framework.Assert.AreEqual(DataType.Number, result.Tuple[1].Type);
			NUnit.Framework.Assert.AreEqual(1, result.Tuple[0].Number);
			NUnit.Framework.Assert.AreEqual(22, result.Tuple[1].Number);
		}
#endif
