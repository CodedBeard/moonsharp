...
namespace MoonSharp.Interpreter.Tests.EndToEnd
{
	...
	public class BinaryDumpTests
	{
		...
		public void BinDump_ChunkDump()
		{
			...
			NUnit.Framework.Assert.AreEqual(81, res.Number);
		}
...
";
			...
			NUnit.Framework.Assert.AreEqual(DataType.Number, res.Type);
			NUnit.Framework.Assert.AreEqual(140, res.Number);
		}
	...
	return 10 * Q.t.dojob();
		...
		public void Load_ChangeEnvWithDebugSetUpvalue()
		{
			...
			NUnit.Framework.Assert.AreEqual(6, list.Count);

			int[] eqs = new int[] { 0, 1, 1, 0, 1, 1 };

			for (int i = 0; i < 6; i++)
				NUnit.Framework.Assert.AreEqual(list[eqs[i]], list[i]);
		}
	}
}
