...
namespace MoonSharp.Interpreter.Tests.EndToEnd
{
	...
	public class StringLibTests
	{
		...
		public void String_GMatch_1()
		{
			...
			NUnit.Framework.Assert.AreEqual("HelloLuauser", res.String);
		}
		...
		public void String_Find_11()
		{
			...
			";
			DynValue res = Script.RunString(script);
			NUnit.Framework.Assert.IsTrue(res.IsNil());
		}
		...
		public void String_GSub_1()
		{
			...
			";
			DynValue res = Script.RunString(script);
			NUnit.Framework.Assert.AreEqual(res.Tuple[0].String, "hello hello world world");
			NUnit.Framework.Assert.AreEqual(res.Tuple[1].Boolean, true);
		}
		...
		public void PrintTest1()
		{
            ...
            NUnit.Framework.Assert.AreEqual("ciao\t1", printed);
		}
    ...
    doesn't match '^[^:]+:%d+: bad argument #1 to 'date' %(invalid conversion specifier '%%Ja'%)'";
		...
		private void TestMatch(string s, string p, bool expected)
		{
			...
			NUnit.Framework.Assert.AreEqual(expected, !res.IsNil());
		}
