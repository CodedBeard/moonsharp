...
namespace NUnit.Framework
{
	...
	public static class NUnit.Framework.Assert
	{
		public static void AreEqualNum(int expected, double other, string message = null)
		{
			NUnit.Framework.Assert.IsTrue((double)expected == other, message ?? string.Format("{0} was expected, {1} was returned", expected, other));
		}
		...
		public static void AreEqual(object expected, object other, string message = null)
		{
			if (expected is int && other is double) 
				...
				NUnit.Framework.Assert.IsTrue(expected.Equals(other), message ?? string.Format("{0} was expected, {1} was returned", expected, other));
			else
				NUnit.Framework.Assert.IsTrue(other == null, message ?? string.Format("null was expected, {0} was returned", other));
		}
		...
		internal static void Catch<ET>(Action a)
		{
			...
			{
				NUnit.Framework.Assert.IsTrue(e is ET);
			}
		}
