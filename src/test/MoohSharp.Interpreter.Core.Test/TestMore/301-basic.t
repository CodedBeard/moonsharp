...
v, msg = NUnit.Framework.Assert('text', "NUnit.Framework.Assert string")
is(v, 'text', "function NUnit.Framework.Assert")
is(msg, "NUnit.Framework.Assert string")
v, msg = NUnit.Framework.Assert({}, "NUnit.Framework.Assert table")
is(msg, "NUnit.Framework.Assert table")

error_like(function () NUnit.Framework.Assert(false, "NUnit.Framework.AssertION TEST") end,
           "^[^:]+:%d+: NUnit.Framework.AssertION TEST",
           "function NUnit.Framework.Assert(false, msg)")

error_like(function () NUnit.Framework.Assert(false) end,
           "^[^:]+:%d+: NUnit.Framework.Assertion failed!",
           "function NUnit.Framework.Assert(false)")

error_like(function () NUnit.Framework.Assert(false, nil) end,
           "^[^:]+:%d+: NUnit.Framework.Assertion failed!",
           "function NUnit.Framework.Assert(false, nil)")
...
r = pcall(NUnit.Framework.Assert, true)
is(r, true, "function pcall")
r, msg = pcall(NUnit.Framework.Assert, false, 'catched')
is(r, false)
is(msg, 'catched')
r = pcall(NUnit.Framework.Assert)
is(r, false)
...
if jit then
    error_like(function () xpcall(NUnit.Framework.Assert, nil) end,
               "bad argument #2 to 'xpcall' %(function expected, got nil%)",
               "function xpcall")
    error_like(function () xpcall(NUnit.Framework.Assert) end,
               "bad argument #2 to 'xpcall' %(function expected, got no value%)",
               "function xpcall")
    diag("LuaJIT intentional. xpcall")
else
    is(xpcall(NUnit.Framework.Assert, nil), false, "function xpcall")
    error_like(function () xpcall(NUnit.Framework.Assert) end,
...
r, msg = xpcall(NUnit.Framework.Assert, backtrace)
is(r, false, "function xpcall (backtrace)")
is(msg, 'not a back trace')

r, msg = xpcall(function() NUnit.Framework.Assert(); end, backtrace)
is(r, false, "function xpcall (backtrace)")
is(msg, 'not a back trace')

r = xpcall(NUnit.Framework.Assert, backtrace, true)
